					...
					<div class="col-md-6">
						<article>
							<div class="project-details">
								<a class="back-icon project-back-page-link "
									onClick="goBack('<?php echo home_url('/') . '/projects/' ?>')">
									<img class="avoid"
										src="<?php echo get_template_directory_uri() . '/images/chevron.svg' ?>" alt="">
									<span>Back</span>
								</a>
	...
	$(document).ready(function() {
		...
		for (let i = 0; i < allProjectImages.length; i++) {
			...
			if (currentImage.dataset.fullUrl) {
				// if(i==0){
				carouselImages +=
					`<img src='${currentImage.dataset.fullUrl}' id="c-media-${i}" data-slide-index="${i}" class="c-media"/>`;
				// }else{
			...
			// }
				...
				carouselImages +=
					`<img src='${currentImage.dataset.src}' id="c-media-${i}" data-slide-index="${i}" class="c-media"/>`;
			}
		}
		...
		$('.project-interior img:not(.avoid)').on('click', function(el) {
			// console.log(el);
			document.querySelector('.p-c-count .current').innerText = parseInt(this.dataset
				.gridSlideIndex) + 1;
